...
def load_all_data():
    ...
    VIP_USERS = {int(k): v for k, v in load_json_from_file("vips.json").items()}
    ...
    try:
        if user:
            if user.id not in USER_INFO_CACHE or (datetime.now(pytz.utc) - USER_INFO_CACHE[user.id].get('timestamp', datetime.min.replace(tzinfo=pytz.utc))).total_seconds() > 3600:
                ...
                approved_date = USER_INFO_CACHE.get(user.id, {}).get('approved_date', datetime.now(pytz.utc).isoformat())
                command_count = USER_INFO_CACHE.get(user.id, {}).get('command_count', 0)
                USER_INFO_CACHE[user.id] = {'first_name': user.first_name,'username': user.username or "N/A",'avatar_path': avatar_path, 'timestamp': datetime.now(pytz.utc), 'command_count': command_count, 'approved_date': approved_date}
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "user":
        ...
        if action_type == "manage":
            ...
            if target_id in VIP_USERS and datetime.fromisoformat(VIP_USERS.get(target_id, '1970-01-01T00:00:00')) > datetime.now(): status += " (VIP)"
        ...
        text = ""
        if action_type == "ban": BANNED_USERS.add(target_id); AUTHORIZED_USERS.discard(target_id); RESTRICTED_USERS.discard(target_id); VIP_USERS.pop(target_id, None); save_to_file("banned_users.txt", BANNED_USERS); save_to_file("authorized_users.txt", AUTHORIZED_USERS); save_to_file("restricted_users.txt", RESTRICTED_USERS); save_vips(); text = f"🚫 User {target_id} has been banned."
...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except (IndexError, ValueError): await update.message.reply_text("⚠️ Usage: <code>/approve [approval_code]</code>", parse_mode=ParseMode.HTML)
...
async def grantvip_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    admin = update.effective_user
    if admin.id not in ADMIN_USERS: return
    await log_user_activity(admin, "/grantvip", context.bot)
    try:
        ...
        await update.message.reply_text("⚠️ Usage: <code>/grantvip [user_id]</code>", parse_mode=ParseMode.HTML)
